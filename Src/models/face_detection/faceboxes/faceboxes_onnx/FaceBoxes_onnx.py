...
from utils_faceboxes.prior_box import PriorBox
from utils_faceboxes.nms_wrapper import nms
from utils_faceboxes.box_utils import decode
from utils_faceboxes.timer import Timer
from utils_faceboxes.config import cfg
from converting_to_onnx import convert_to_onnx
...
def viz_bbox(img, dets,scores, wfp='out.jpg'):
    # show
    for b,score in zip(dets,scores):
        if score < vis_thres:
            continue
...
class FaceBoxes_ONNX(object):
    def __init__(self, timer_flag=False):
        ...
        self.session = onnxruntime.InferenceSession(onnx_path, None)# providers=['CUDAExecutionProvider'])
...
def main():
    ...
    dets,scores = face_boxes(img)  # xmin, ymin, w, h
    # print(dets)

    # repeating inference for `n` times
    n = 10
    for i in range(n):
        dets,scores = face_boxes(img)

    print(dets)
    wfn = fn.replace('.jpg', '_det.jpg')
    wfp = osp.join('../examples/results', wfn)
    viz_bbox(img, dets,scores, wfp)
