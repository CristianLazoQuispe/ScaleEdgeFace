...
class YOLOv8_face:
    def __init__(self, model_name, conf_thres=0.45, iou_thres=0.5):
        ...
        self.path_model = os.path.join(self.my_path, "weights",model_name)
        self.net = cv2.dnn.readNet(self.path_model)
    ...
    def predict(self,srcimg):
        srcimg.flags.writeable = False
        det_bboxes, det_conf, det_classid, landmarks = self.detect(srcimg)
        boxes = []
        for box in det_bboxes:
            boxes.append([box[0],box[1],box[0]+box[2],box[1]+box[3]])
        #print(det_bboxes,"boxes ",boxes)
        srcimg.flags.writeable = True  
        return boxes, det_conf
